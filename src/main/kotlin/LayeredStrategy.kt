...
class LayeredStrategy(val model: Model) : Strategy {
    ...
    private fun reachableFrom(start: Coord): Sequence<Coord> {
        val reachable = HashSet<Coord>()

        val testAndAdd = { x: Int, y: Int, z: Int ->
            if (!state.matrix[x, y, z]) {
                reachable.add(Coord(x, y, z))
                false
            } else {
                true
            }
        ...
        for (x in min(start.x + 1, r) .. min(start.x + 15, r)) {
            if (testAndAdd(x, start.y, start.z)) break

            if (x <= start.x + 5) {
                for (y in min(start.y + 1, r) .. min(start.y + 5, r)) {
                    if (testAndAdd(x, y, start.z)) break
                }
                for (y in max(start.y - 1, 0) downTo max(start.y - 5, 0)) {
                    if (testAndAdd(x, y, start.z)) break
                }
                for (z in min(start.z + 1, r) .. min(start.z + 5, r)) {
                    if (testAndAdd(x, start.y, z)) break
                }
                for (z in max(start.z - 1, 0) downTo max(start.z - 5, 0)) {
                    if (testAndAdd(x, start.y, z)) break
                }
            }
        }

        for (x in max(start.x - 1, 0) downTo max(start.x - 15, 0)) {
            if (testAndAdd(x, start.y, start.z)) break
        ...
        for (y in min(start.y + 1, r) .. min(start.y + 15, r)) {
            if (testAndAdd(start.x, y, start.z)) break

            if (y <= start.y + 5) {
                for (x in min(start.x + 1, r) .. min(start.x + 5, r)) {
                    if (testAndAdd(x, y, start.z)) break
                }
                for (x in max(start.x - 1, 0) downTo max(start.x - 5, 0)) {
                    if (testAndAdd(x, y, start.z)) break
                }
                for (z in min(start.z + 1, r) .. min(start.z + 5, r)) {
                    if (testAndAdd(start.x, y, z)) break
                }
                for (z in max(start.z - 1, 0) downTo max(start.z - 5, 0)) {
                    if (testAndAdd(start.x, y, z)) break
                }
        ...
        for (y in max(start.y - 1, 0) .. max(start.y - 15, 0)) {
            if (testAndAdd(start.x, y, start.z)) break
        ...
        for (z in min(start.z + 1, r) .. min(start.z + 15, r)) {
            if (testAndAdd(start.x, start.y, z)) break
        ...
        for (z in start.z - 1 downTo max(start.z - 15, 0)) {
            if (testAndAdd(start.x, start.y, z)) break
